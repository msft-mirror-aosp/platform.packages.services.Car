...
public class VoiceAssistEventHandler implements
    ...
    public void onKeyEvents(int targetDisplayType,
        ...
        logIfDebuggable("Received a key event");
    ...
    private void launchUserTosRedirectActivity() {
        ...
        if (targetDisplayId == Display.INVALID_DISPLAY) {
            logIfDebuggable("No main display found; Not launching user TOS redirect activity");
            return;
        }
        logIfDebuggable("Launching user TOS on display id {" + targetDisplayId + "}");
    ...
    private int getDisplayIdForDisplayType(int targetDisplayType) {
        ...
        logIfDebuggable("Resolved display id {" + displayId + "} for display type {"
                + targetDisplayType + "}");
        return displayId;
    }
}