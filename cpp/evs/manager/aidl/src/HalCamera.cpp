...
std::string HalCamera::toString(Stream configuration, const char* indent) {
    ...
    StringAppendF(&streamInfo,
                  ...
                  double_indent.data(), static_cast<uint32_t>(configuration.format),
                  double_indent.data(), static_cast<uint64_t>(configuration.usage),
                  double_indent.data(), static_cast<uint32_t>(configuration.rotation));
